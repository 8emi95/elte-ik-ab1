-- Usage @isBinaryXMLMigrationAllowed.sql <ASM Table Space>
-- Determine if a conversion from OR to Binary is required (db 11.2.0.4 and up). If so, check if new Table Space has ASM setting.
-- Example: @isBinaryXMLMigrationAllowed.sql <ASM Table Space>

WHENEVER SQLERROR EXIT SQL.SQLCODE;

EXECUTE dbms_output.put_line('Start determine if Binary xml migration is allowed. ' || systimestamp);
...
BEGIN
  ...
  -- Query to determine what version of workflow xml schema is currently installed in repository
    ...
    EXCEPTION WHEN NO_DATA_FOUND THEN
      ...
      COMMIT;
    END;
    IF (v_wf_num != '11.2.2.1.1') THEN -- only check for migration if schema is not equal to schema contained in this release
     dbms_output.put_line('XML Schema migration required.');  
      SELECT SEGMENT_SPACE_MANAGEMENT INTO v_segment FROM dba_tablespaces WHERE tablespace_name = '&TABLE_SPACE';
      IF (v_segment != 'AUTO') THEN
        RAISE_APPLICATION_ERROR(-20000, 'Table space with ASM setting is required.');
      END IF;     
    ELSE
...
END;
/
EXECUTE dbms_output.put_line('Finished determine if Binary xml migration is allowed. ' || systimestamp);
