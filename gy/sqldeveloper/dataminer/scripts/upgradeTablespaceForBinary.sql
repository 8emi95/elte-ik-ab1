...
DEFINE ASM_TABLESPACE = &1

ALTER USER ODMRSYS quota 100M on &ASM_TABLESPACE;

-- create a WORKFLOW_DATA_BACKUP column (Binary Storage in ASM) to backup existing workflows
ALTER TABLE "ODMRSYS"."ODMR$WORKFLOWS" ADD(WORKFLOW_DATA_BACKUP SYS.XMLTYPE)
  XMLTYPE COLUMN WORKFLOW_DATA_BACKUP STORE AS SECUREFILE BINARY XML (TABLESPACE &ASM_TABLESPACE);
