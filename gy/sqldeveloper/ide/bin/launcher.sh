...
Resolve()
{
  ...
  echo "$*" | sed -e "s|^[.]/|$PWD/|; s|^[.][.]/|$PWD/../|; s|\([^-./$\w]\)[.]/|\1$PWD/|g; s|\([^-./$\w]\)[.][.]/|\1$PWD/../|g"
}
...
AddVMOptionHotspot6()
{
  APP_VM_OPTS_HOTSPOT6[${#APP_VM_OPTS_HOTSPOT6[*]}]="`Resolve $*`"
}
AddVMOptionHotspot7()
{
  APP_VM_OPTS_HOTSPOT7[${#APP_VM_OPTS_HOTSPOT7[*]}]="`Resolve $*`"
}
      ...
      then
        ...
        then
          ...
          then
            ...
            # See if we can find /usr/java6*
            jdk=`ls /usr 2>/dev/null | grep java6 | sort -r | head -1`
            if [ -f "/usr/$jdk/bin/java" ]
            then
...
CheckJDK()
{
  ...
  then
    ...
    then
      JAVA="${APP_JAVA_HOME}/bin/java -Xbootclasspath/a:${APP_JAVA_HOME}/lib/tools.jar:${APP_JAVA_HOME}/lib/dt.jar "
      if [ "X$APP_JAVA_VM" != "X" ]
      then
...
#
# This method will add all the specific VM Options,  we support 4 VM specific options:
#    JRockit       <-- JRockit VM specific option
#    Hotspot       <-- Hotspot VM specific option
#    Hotspot6      <-- Jdk 6 Hotspot VM specific option
#    Hotspot7      <-- Jdk 7 Hotspot VM specific option
#
# For OS/X, we assume that we only support hotspot and release 6 of it.   That is true
# for Abrams anyway.   
#
AppendVMSpecificOptions()
{
  local darwin=`uname | grep Darwin`
  local hotspotVM=`ls ${APP_JAVA_HOME}/jre/lib/*/{server,client,hotspot} 2> /dev/null`
  local jrockitVM=`ls ${APP_JAVA_HOME}/jre/lib/*/jrockit 2> /dev/null`
  if [ "X$hotspotVM" != "X" ] && [ "X$jrockitVM" = "X" ] || [ "X$darwin" = "XDarwin" ]
  then
    for (( i = 0; i < ${#APP_VM_OPTS_HOTSPOT[@]}; i++ )); do
      AddVMOption "${APP_VM_OPTS_HOTSPOT[$i]}"
    done
    local hotspotVMVersion=`grep javase/[67]/docs ${APP_JAVA_HOME}/jre/Welcome.html 2> /dev/null`
    local hotspotVMVersion=`expr "$hotspotVMVersion" : '.*javase/\([67]\)/docs.*'`
	[ "X$darwin" = "XDarwin" ] && hotspotVMVersion="6"
    case $hotspotVMVersion in
      6) for (( i = 0; i < ${#APP_VM_OPTS_HOTSPOT6[@]}; i++ )); do
           AddVMOption "${APP_VM_OPTS_HOTSPOT6[$i]}"
         done ;;
      7) for (( i = 0; i < ${#APP_VM_OPTS_HOTSPOT7[@]}; i++ )); do
           AddVMOption "${APP_VM_OPTS_HOTSPOT7[$i]}"
         done ;;
    esac
  fi
...
LaunchIDE()
{
        ...
        case $1 in 
          --*)
            ...
            case $NEWARG in
              ...
              addvmoptionhotspot6) NEWARG="AddVMOptionHotspot6" ;;
              addvmoptionhotspot7) NEWARG="AddVMOptionHotspot7" ;;
            esac
            $NEWARG "$NEWOPD"
    ...
    then
            ...
            if [ `expr "$s" : 'java version \"1\.[0-7].*\"'` -le 0 ]
            then
